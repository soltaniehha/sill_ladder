...
template<class T>
void
init_time_evolution(SystemThermo<T> &S, BasicOp<T> *_op1 = NULL, BasicOp<T> *_op2 = NULL, bool propagate_gs = true, bool evolve_gs = false, bool clear_corr = true)
{
  int lx = S.h.lattice().size();

  S.set_imaginary_time(false);
  if(propagate_gs && !(_op1 == NULL)) S.propagate_gs(S.gs,evolve_gs);
  S.phase = S.energy[0];
  S.dm_ops.clear();
...
template<class T>
void
init_time_evolution(SystemThermo<T> &S, BasicOp<T> *_op1 = NULL, Hami<T> *new_corr = NULL, bool propagate_gs = true, bool evolve_gs = false, bool clear_corr = true)
{
  int lx = S.h.lattice().size();
...
template<class T>
void
init_time_evolution(SystemThermo<T> &S, Hami<T> *h = NULL, Hami<T> *new_corr = NULL, bool propagate_gs = true, bool evolve_gs = false, bool clear_corr = true)
{
  int lx = S.h.lattice().size();

  S.set_imaginary_time(false);
  if(propagate_gs && !(h == NULL)) S.propagate_gs(S.gs,evolve_gs);
  S.phase = S.energy[0];
  S.dm_ops.clear();
  S.set_store_products(false);
...
// Suzuki-Trotter
    for(int term = 1; term <= nexp; term++){
      ...
      hij = build_st_ops(S.st_hami, xtstep, S.imaginary_time());
...
// Run an empty sweep and measure correlations
    ...
    if(!S.imaginary_time() && S.rotate_terms()){
      ...
      aux = product_default(S, S.gs, &S.h);
      T xe = product(S.gs,aux);
      cout << "ENER T= " << xtime << " " << real(xe)/(real(product(S.gs,S.gs))) << " " << log(real(product(S.gs,S.gs))) << endl;
      S.measure();
    }
