...
class BasicOp: public dmtk::BMatrix<T>
{
  ...
  public:
    ...
    BasicOp(const char *name, const Matrix<T> &_m, const Basis& b, int site, int internal_site = 0, int label = -1):
       _name(name), BMatrix<T>(b), _site(site), _internal_site(internal_site), _label(label), _type(OP_SYSTEM), _is_term(false), _is_hami(false), _is_fermion(false), _is_diagonal(true) {}
