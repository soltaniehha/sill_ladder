...
class SystemThermo:public System<T>
{
  ...
  protected:
    void init_signals_thermo()
    {
       this->_signal_id.push_back(std::string("SYSTEM_THERMO_SIGNAL_EVOLVE"));
    }
  public:
    Hami<T> st_hami;
    BasicOp<T> hij; 
    ...
    SystemThermo(const Hami<T> &_h, const Lattice& lattice, double _dt, const char *name):System<T>(_h, lattice, name), _evolve_gs(false), _propagate_gs(false), _apply_product_time(false), time_evolve(false), _imaginary_time(true), my_time(0), phase(0), _n_lanczos(-1), _use_rk(true), _in_empty_sweep(false)
      { hsites.resize(lattice.size()); dt = _dt; init_signals_thermo(); }
    ...
    void time_sweep(size_t t, size_t _dir = RIGHT2LEFT, size_t _start = 0, bool build_operators = true);
    void time_sweep1(size_t t, size_t _dir = RIGHT2LEFT, size_t _iter = 1, bool build_operators = true);
...
{
  my_time += dt;
  this->signal_emit(std::string("SYSTEM_THERMO_SIGNAL_EVOLVE"));
}
#endif //USE_RK
...
*/
      _S::read_rho(rho1, basis1, _S::iter, LEFT);
      _S::read_gs(_S::gs, _S::iter-1, LEFT);
      _S::seed = _S::gs;
  ...
  for(_S::iter = 1; _S::iter <= ls-3; _S::iter++)
  {
    ...
    if(_S::iter != 1){
      ...
      _S::read_rho(rho1, basis1, _S::iter, RIGHT);
      _S::read_rho(rho2, basis2, _S::lattice().ls()-_S::iter-2+1, LEFT);
      _S::read_gs(_S::gs, _S::iter-1, RIGHT);
      cout << "NEW SEED" << endl;
  ...
  for(_S::iter = 1; _S::iter <= ls-3; _S::iter++)
  {
    ...
    if(_S::iter != 1){
      ...
      _S::read_rho(rho1, basis1, _S::iter, LEFT);
      _S::read_rho(rho2, basis2, _S::lattice().ls()-_S::iter-2+1, RIGHT);
      _S::read_gs(_S::gs, _S::iter-1, LEFT);
...
template<class T>
void
SystemThermo<T>::time_sweep1(size_t t, size_t _dir, size_t _iter, bool build_operators)
{
  int ls = _S::lattice().ls();
  CTimer clock;
...
template<class T>
void
SystemThermo<T>::time_sweep(size_t t, size_t _dir, size_t _start, bool build_operators)
{
  int ls = _S::lattice().ls();
  CTimer clock;
...
SystemThermo<T>::start_sweep(const BasicOp<T> *op, size_t t, size_t _dir, size_t _start, size_t _block)
{
  ...
  if(_S::iter == 1)
#ifdef USE_RK
    _S::read_gs(_S::gs, ls-2-_S::iter, _dir == LEFT2RIGHT ? RIGHT : LEFT);
#else
    _S::read_gs(_S::gs, ls-1-_S::iter, _dir == LEFT2RIGHT ? RIGHT : LEFT);
#endif
  else
    _S::read_gs(_S::gs, _S::iter-1, _dir == LEFT2RIGHT ? LEFT : RIGHT);
...
//  _seed = _seed / sqrt(product(_seed,_seed));
  ...
  if(_start == 1)
#ifdef USE_RK
    _S::write_gs(_S::gs, ls-2-_S::iter, _dir == LEFT2RIGHT ? RIGHT : LEFT);
#else
    _S::write_gs(_S::gs, ls-1-_S::iter, _dir == LEFT2RIGHT ? RIGHT : LEFT);
#endif
  else
...
//#endif
  ...
  if(rotate_states){
    ...
    if(_S::iter <= 1 || _S::iter >= ls-2){
      if(_S::iter == 0)
        _S::read_gs(_S::gs, ls-2-_S::iter, _S::dir == LEFT2RIGHT ? RIGHT : LEFT);
      else
#ifdef USE_RK
        _S::read_gs(_S::gs, ls-2-_S::iter, _S::dir == LEFT2RIGHT ? RIGHT : LEFT);
#else
        if(_in_empty_sweep)
          _S::read_gs(_S::gs, ls-1-_S::iter, _S::dir == LEFT2RIGHT ? RIGHT : LEFT);
        else 
          _S::read_gs(_S::gs, _S::iter-1, _S::dir == LEFT2RIGHT ? LEFT : RIGHT);
//        _S::_target[first_state] = _S::gs;
#endif
    } else {
      if(_S::dir == LEFT2RIGHT){
