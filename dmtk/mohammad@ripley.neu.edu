// Mohammad-note: deavtivated lanczos_unsymmetric_modified
...
//      x1[0] = 1.;
    ...
    size_t nmax = std::min(999, (int)gs(0).size());
...
//      cout << "Iter=" << iter << " " << a(iter) << " " << b(iter) << endl;
      ...
      if(maxiter > 0) {  // We are building the ground state;
        ...
        if(iter >= 2){
          ...
          if((force_maxiter && iter >= control_max) || (iter >= gs(0).size() || iter == 999 || fabs(b(iter)) < tol) || (force_maxiter == false && fabs(eini-e0) <= tol))
          { 
             // converged
...
//      x1[0] = 1.;
    ...
    size_t nmax = std::min(999, (int)gs.size());
    for(iter = 1; iter <= nmax; iter++){ // Lanczos iteration
      eini = e0;
...
//      cout << "Iter=" << iter << " " << a(iter) << " " << b(iter) << endl;
      ...
      if(maxiter > 0) {  // We are building the ground state;
        ...
        if(iter >= 2){
          ...
          cout << setprecision(12) << "Iter = " << iter << "  Ener = " << e0 << endl;
          if((force_maxiter && iter >= control_max) || (iter >= gs.size() || iter == 999 || fabs(b(iter)) < tol) || (!force_maxiter  && fabs(eini-e0) <= tol))
          { 
             // converged
             cout << setprecision(12) << "E0 = " << e0 << endl;
             maxiter = iter;
